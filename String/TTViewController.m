...
//    3. Создайте массив, каждый элемент которого является предложением данного текста.
    ...
    NSRange range1 = {0,[text length]/2};
    NSString *txtfirstHalf = [text substringWithRange:range1];
    NSLog(@"%@",txtfirstHalf);
    
    NSString *txtsecondHalf = [text substringFromIndex:[text length]/2];
    NSLog(@"%@",txtsecondHalf);
    
    NSArray *sentence = [[NSArray alloc]init];
    sentence = [text componentsSeparatedByString:@"."];
    NSLog(@"%@",sentence);
...
//
    ...
    for (NSString *str in sentence) {
        
        NSRange range = [str rangeOfString:@"NSString"];
        
        if (range.location != NSNotFound) {
            [arraySentence addObject:str];
        }
